...
#include "stdafx.h"
#include <GL/glut.h>
#include <stdlib.h>								// N = 14
#include <stdio.h>
#include <math.h>
#include <GL/glext.h>

const GLint i2_14 = pow(2.0, 14);
const GLint i2_13 = pow(2.0, 13);
const GLint i2_12 = pow(2.0, 12);
...
GLint RECT_LIST;
GLint TEXTURE_16384_LIST, TEXTURE_8192_LIST, TEXTURE_4096_LIST;
...
void DisplayScene()
{
	...
	glCallList(TEXTURE_16384_LIST);
	glCallList(TEXTURE_8192_LIST);
...
void GenerateDisplayLists()
{
	...
	// sprawdzenie czy implementacja biblioteki obsługuje tekstury o wymiarze 16384
	GLint size;
	glGetIntegerv(GL_MAX_TEXTURE_SIZE, &size);
	if (size < i2_14)
	{
		printf("Rozmiar tekstur mniejszy od 16384");
		exit(0);
	}

	// dane tekstury
	GLubyte texture[16384 * 3];

	// przygotowanie danych tekstury RGB
	for (int i = 0; i < i2_14; i++)
	{
		texture[3 * i + 0] = i / 64;
		texture[3 * i + 1] = i / 64;
		texture[3 * i + 2] = i / 64;
	...
	// generowanie identyfikatora listy wyświetlania
	TEXTURE_16384_LIST = glGenLists(1);

	// lista wyświetlania - tekstura o szerokości 16384 tekseli
	glNewList(TEXTURE_16384_LIST, GL_COMPILE);

	// definiowanie tekstury
	glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB, 16384, 0, GL_RGB, GL_UNSIGNED_BYTE, texture);
	...
	for (int i = 0; i < i2_13; i++)
	{
		texture[i] = i / 32;
	}

	// generowanie identyfikatora listy wyświetlania
	TEXTURE_8192_LIST = glGenLists(1);

	// lista wyświetlania - tekstura o szerokości 8192 tekseli
	glNewList(TEXTURE_8192_LIST, GL_COMPILE);

	// definiowanie tekstury
	glTexImage1D(GL_TEXTURE_1D, 0, GL_LUMINANCE, 8192, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, texture);
	...
	// lista wyświetlania - tekstura o szerokości 4096 tekseli
	glNewList(TEXTURE_4096_LIST, GL_COMPILE);

	// definiowanie tekstury
	glTexImage1D(GL_TEXTURE_1D, 0, GL_INTENSITY, 4096, 0, GL_RGB, GL_UNSIGNED_BYTE, texture);
