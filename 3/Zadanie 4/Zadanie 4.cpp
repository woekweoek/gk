// Zadanie 4.cpp : Defines the entry point for the console application.
//
...
void GenerateTextures()
{
	...
	// wczytanie tekstury white_window.tga
	error = load_targa("white_window.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku white_window.tga");
		exit(0);
	}
...
void GenerateDisplayLists()
{
	...
	// czwarta lista wyświetlania - okna
	glNewList(WINDOW_LIST, GL_COMPILE);

	//  14 czworokątow
	glBegin(GL_QUADS);

	// przednia ściana : 4 okna
	glTexCoord2f(0.0, 0.0);
	...
	// prawa ściana : 4 okna
	glTexCoord2f(0.0, 0.0);
	glVertex3f(1.01, -0.75, 0.75);
	glTexCoord2f(1.0, 0.0);
	glVertex3f(1.01, -0.75, 0.25);
	glTexCoord2f(1.0, 1.0);
	glVertex3f(1.01, -0.25, 0.25);
	glTexCoord2f(0.0, 1.0);
	glVertex3f(1.01, -0.25, 0.75);
	...
	// lewa ściana : 2 okna
	glTexCoord2f(0.0, 0.0);
	glVertex3f(-1.01, -0.5, 0.75);
	glTexCoord2f(1.0, 0.0);
	glVertex3f(-1.01, -0.5, 0.25);
	glTexCoord2f(1.0, 1.0);
	glVertex3f(-1.01, 0.5, 0.25);
	glTexCoord2f(0.0, 1.0);
	glVertex3f(-1.01, 0.5, 0.75);
